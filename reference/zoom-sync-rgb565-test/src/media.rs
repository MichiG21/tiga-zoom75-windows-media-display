...
/// Encode an image as true 16-bit RGB565 (2 bytes per pixel).
///
/// The Zoom75 Tiga board implementation documents its static image payload as
/// RGB565, so unlike `encode_image` this function does NOT append the legacy
/// 0xFF alpha byte used by the Zoom65-style media format.
pub fn encode_image_rgb565(
    image: DynamicImage,
    background: [u8; 3],
    nearest: bool,
    width: u32,
    ...
    print!("resizing and encoding image as RGB565 (2 Bpp) ... ");
    stdout().flush().unwrap();
    let [br, bg, bb] = background;
    ...
    let buf = image
        ...
        .flat_map(|p| {
            ...
            // Composite transparent pixels against the requested background.
            let a = a as f64 / 255.0;
            let ba = 1. - a;
            r = ((br as f64 * ba) + (r as f64 * a)) as u8;
            g = ((bg as f64 * ba) + (g as f64 * a)) as u8;
            ...
            // Big-endian RGB565: exactly two bytes per pixel.
            rgb565::Rgb565::from_rgb888_components(r, g, b).to_rgb565_be()
        })
        .collect::<Vec<_>>();

    debug_assert_eq!(buf.len(), (width * height * 2) as usize);
