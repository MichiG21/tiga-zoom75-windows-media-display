...
use crate::media::{encode_gif, encode_image, encode_image_rgb565};
                ...
                match set_command {
                    ...
                    SetCommand::Image(args) => match args {
                        SetMediaArgs::Set { nearest, path, bg } => {
                            ...
                            // Re-encode and upload to keyboard. The Zoom75 Tiga
                            // protocol documents static image data as true RGB565
                            // (2 bytes/pixel); the legacy encoder appends an extra
                            // 0xFF byte and is kept for the other boards.
                            let is_tiga = board.info().cli_name == "zoom75-tiga";
                            let encoded = if is_tiga {
                                println!("Zoom75 Tiga detected: using true RGB565 (2 bytes/pixel)");
                                encode_image_rgb565(image, bg.0, nearest, width, height)
                            } else {
                                encode_image(image, bg.0, nearest, width, height)
                            }
                            .ok_or("failed to encode image")?;
                            let len = encoded.len();
                            let chunk_size = if is_tiga { 16 } else { 24 };
